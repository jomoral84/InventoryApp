...
export default function SeguimientoPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    if (filterDeleg && user?.role === "admin")
      params.set("delegacion", filterDeleg);
  ...
  const delegaciones = Array.from(
    new Set(logs.map((l: any) => l.delegacion)),
  ).sort() as string[];
  ...
  return (
    <>
      <Head>
        <title>Seguimiento — Inventario CNRT</title>
      </Head>
          ...
          {fetching ? (
            ...
            <div className="text-center py-20 bg-white rounded-2xl border border-blue-100">
              ...
              <p className="text-blue-300 text-sm mt-1">
                Los eventos se registran automáticamente al dar de alta, editar
                ...
                <thead className="bg-blue-50/70 border-b border-blue-100">
                  <tr>
                    {[
                      "Fecha y hora",
                      "Acción",
                      user.role === "admin" ? "Delegación" : null,
                      "Categoría",
                      "Descripción",
                      "Usuario",
                    ]
                      ...
                      .map((h) => (
                        <th
                          key={h!}
                          className="text-left text-xs font-semibold text-blue-400 uppercase tracking-wider px-4 py-3 whitespace-nowrap"
                        >
                          {h}
                        </th>
                      ))}
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-xs text-blue-500">
                          {log.itemTipo
                            ? TIPO_LABEL[log.itemTipo] || log.itemTipo
                            : "—"}
                        </td>
                        <td className="px-4 py-3 text-sm text-blue-800 max-w-sm">
                          {log.descripcion}
                        </td>
