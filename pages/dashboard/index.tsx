...
function TH({ children }: { children: React.ReactNode }) {
  return <th className="text-left text-xs font-semibold text-blue-400 uppercase tracking-wider px-4 py-3 whitespace-nowrap">{children}</th>
}
function TD({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <td className={`px-4 py-3 text-sm text-blue-800 ${className}`}>{children}</td>
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (!loading && user?.role !== 'admin') { router.replace('/inventario'); return }
    fetchItems()
  }, [user, loading])
  ...
  const items = allItems.filter(i =>
    i.tipo === activeTab && (!filterDelegacion || i.delegacion === filterDelegacion)
  )
  const delegaciones = [...new Set(allItems.map((i: any) => i.delegacion))].sort() as string[]
  const counts = TABS.reduce((acc, t) => ({ ...acc, [t.key]: allItems.filter((i: any) => i.tipo === t.key).length }), {} as Record<string, number>)
  ...
  return (
    <>
      <Head><title>Dashboard — Inventario</title></Head>
      <Layout title="Dashboard">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {TABS.map(t => (
            <div key={t.key} className="bg-white rounded-xl border border-blue-100 p-4 flex items-center gap-3">
              <span className="text-2xl">{t.icon}</span>
              <div>
                <p className="text-2xl font-bold text-blue-900">{counts[t.key] || 0}</p>
        ...
        <div className="bg-white rounded-2xl border border-blue-100 overflow-hidden shadow-sm">
            ...
            {TABS.map(tab => (
              <button
                ...
                className={`flex items-center gap-2 px-6 py-4 text-sm font-semibold transition-all relative border-r border-blue-100/50 last:border-r-0 ${
                  activeTab === tab.key ? 'text-blue-700 bg-white tab-active' : 'text-blue-400 hover:text-blue-600'
                }`}
              >
                <span>{tab.icon}</span>
                <span>{tab.label}</span>
                <span className={`ml-1 text-xs px-1.5 py-0.5 rounded-full font-medium ${activeTab === tab.key ? 'bg-blue-100 text-blue-700' : 'bg-blue-100/50 text-blue-400'}`}>
                  {counts[tab.key] || 0}
                </span>
              </button>
            ))}
          ...
          {showNewForm && (
            <div className="border-b border-blue-100 bg-blue-50/30 px-6 py-5 fade-in">
              ...
              <div className="max-w-2xl">
                ...
                {activeTab === 'alcoholimetro' && <FormAlcoholimetro onSuccess={handleSaved} />}
                {activeTab === 'pda_celular'  && <FormPdaCelular  onSuccess={handleSaved} />}
                {activeTab === 'informatica'  && <FormInformatica  onSuccess={handleSaved} />}
              </div>
            </div>
          )}
          ...
          <div className="overflow-x-auto">
            {fetching ? (
              ...
              <MovilTable items={items} onDelete={handleDelete} onImg={setImgModal} onEdit={setEditItem} />
            ) : activeTab === 'alcoholimetro' ? (
              <AlcoholimetroTable items={items} onDelete={handleDelete} onEdit={setEditItem} />
            ) : activeTab === 'pda_celular' ? (
              <PdaTable items={items} onDelete={handleDelete} onEdit={setEditItem} />
            ) : (
              <InformaticaTable items={items} onDelete={handleDelete} onEdit={setEditItem} />
            )}
          </div>
        </div>
  ...
  return (
    <table className="w-full">
      <thead className="bg-blue-50/70 border-b border-blue-100">
        <tr><TH>Delegación</TH><TH>Usuario</TH><TH>Dominio</TH><TH>Marca/Modelo</TH><TH>Año</TH><TH>Km</TH><TH>Estado</TH><TH>Mat.</TH><TH>RTO</TH><TH>Observaciones</TH><TH>Foto</TH><TH>Fecha</TH><TH>Acciones</TH></tr>
      </thead>
      <tbody className="divide-y divide-blue-50">
        {items.map((i: any) => (
          <tr key={i.id} className="hover:bg-blue-50/30 transition-colors">
            <TD><span className="font-medium">{i.delegacion}</span></TD>
            <TD className="text-blue-500">{i.username}</TD>
            <TD><span className="font-mono text-blue-700 font-semibold">{i.dominio}</span></TD>
            <TD>{i.marca} {i.modelo}</TD>
            <TD>{i.anio}</TD>
            <TD>{i.kilometros}</TD>
            <TD><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${estadoBadge(i.estado)}`}>{i.estado}</span></TD>
            <TD><span className={i.matafuegos === 'Si' ? 'text-green-600' : 'text-red-500'}>{i.matafuegos}</span></TD>
            <TD><span className={i.rto === 'Si' ? 'text-green-600' : 'text-red-500'}>{i.rto}</span></TD>
            <TD className="max-w-[120px] truncate text-blue-400 text-xs">{i.observaciones || '—'}</TD>
            <TD>{i.fotoUrl ? <button onClick={() => onImg(i.fotoUrl)} className="text-blue-500 hover:text-blue-700 text-xs underline">Ver foto</button> : '—'}</TD>
            <TD className="text-blue-400 text-xs">{new Date(i.createdAt).toLocaleDateString('es-AR')}</TD>
            <ActionButtons onEdit={() => onEdit(i)} onDelete={() => onDelete(i.id)} />
          </tr>
        ))}
      </tbody>
    </table>
...
function AlcoholimetroTable({ items, onDelete, onEdit }: any) {
  return (
    <table className="w-full">
      <thead className="bg-blue-50/70 border-b border-blue-100">
        <tr><TH>Delegación</TH><TH>Usuario</TH><TH>N° Serie</TH><TH>Estado</TH><TH>Vencimiento</TH><TH>Observaciones</TH><TH>Fecha carga</TH><TH>Acciones</TH></tr>
      </thead>
      <tbody className="divide-y divide-blue-50">
        {items.map((i: any) => (
          <tr key={i.id} className="hover:bg-blue-50/30">
            <TD><span className="font-medium">{i.delegacion}</span></TD>
            <TD className="text-blue-500">{i.username}</TD>
            <TD><span className="font-mono text-blue-700">{i.nroSerie}</span></TD>
            <TD><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${estadoBadge(i.estado)}`}>{i.estado}</span></TD>
            <TD>{i.fechaVencimiento || '—'}</TD>
            <TD className="max-w-[160px] truncate text-blue-400 text-xs">{i.observaciones || '—'}</TD>
            <TD className="text-blue-400 text-xs">{new Date(i.createdAt).toLocaleDateString('es-AR')}</TD>
            <ActionButtons onEdit={() => onEdit(i)} onDelete={() => onDelete(i.id)} />
          </tr>
        ))}
...
function PdaTable({ items, onDelete, onEdit }: any) {
  return (
    <table className="w-full">
      <thead className="bg-blue-50/70 border-b border-blue-100">
        <tr><TH>Delegación</TH><TH>Usuario</TH><TH>Estado</TH><TH>Marca/Modelo</TH><TH>N° Sistemas</TH><TH>N° Línea</TH><TH>Empresa</TH><TH>Asignada a</TH><TH>Obs.</TH><TH>Fecha</TH><TH>Acciones</TH></tr>
      </thead>
      <tbody className="divide-y divide-blue-50">
        {items.map((i: any) => (
          <tr key={i.id} className="hover:bg-blue-50/30">
            ...
            <TD>{i.empresa}</TD>
            <TD>{i.asignadaA || '—'}</TD>
            <TD className="max-w-[100px] truncate text-blue-400 text-xs">{i.observaciones || '—'}</TD>
            <TD className="text-blue-400 text-xs">{new Date(i.createdAt).toLocaleDateString('es-AR')}</TD>
            <ActionButtons onEdit={() => onEdit(i)} onDelete={() => onDelete(i.id)} />
          </tr>
        ))}
...
function InformaticaTable({ items, onDelete, onEdit }: any) {
  return (
    <table className="w-full">
      <thead className="bg-blue-50/70 border-b border-blue-100">
        <tr><TH>Delegación</TH><TH>Usuario</TH><TH>Dispositivo</TH><TH>Estado</TH><TH>Marca/Modelo</TH><TH>N° Sistemas</TH><TH>Observaciones</TH><TH>Fecha</TH><TH>Acciones</TH></tr>
      </thead>
      <tbody className="divide-y divide-blue-50">
        {items.map((i: any) => (
          <tr key={i.id} className="hover:bg-blue-50/30">
            <TD><span className="font-medium">{i.delegacion}</span></TD>
            <TD className="text-blue-500">{i.username}</TD>
            <TD><span className="font-semibold text-blue-700">{i.dispositivo}</span></TD>
            <TD><span className={`text-xs px-2 py-0.5 rounded-full font-medium ${estadoBadge(i.estado)}`}>{i.estado}</span></TD>
            <TD>{i.marca} {i.modelo}</TD>
            <TD className="font-mono text-xs">{i.nroSistemas || '—'}</TD>
            <TD className="max-w-[140px] truncate text-blue-400 text-xs">{i.observaciones || '—'}</TD>
            <TD className="text-blue-400 text-xs">{new Date(i.createdAt).toLocaleDateString('es-AR')}</TD>
            <ActionButtons onEdit={() => onEdit(i)} onDelete={() => onDelete(i.id)} />
          </tr>
        ))}
