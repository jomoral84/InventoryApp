...
export default function UserDashboardPage() {
  ...
  const items = allItems.filter(i => i.tipo === activeTab)
  const counts = TABS.reduce(
    (acc, t) => ({ ...acc, [t.key]: allItems.filter(i => i.tipo === t.key).length }),
  ...
  return (
    <>
      <Head><title>Mi Delegación — Inventario</title></Head>
      <Layout title={`Delegación ${user.delegacion}`}>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {TABS.map(t => (
            <div
              key={t.key}
              className="bg-white rounded-xl border border-blue-100 p-4 flex items-center gap-3 cursor-pointer hover:border-blue-300 transition-colors"
  ...
  return (
    <table className="w-full">
      <thead className="bg-blue-50/70 border-b border-blue-100">
        <tr>
          <TH>Cargado por</TH><TH>Dominio</TH><TH>Marca/Modelo</TH>
          <TH>Año</TH><TH>Km</TH><TH>Estado</TH>
          <TH>Mat.</TH><TH>RTO</TH><TH>Observaciones</TH>
...
function AlcoholimetroTable({ items, onDelete, onEdit }: any) {
  return (
    <table className="w-full">
      <thead className="bg-blue-50/70 border-b border-blue-100">
        <tr>
          <TH>Cargado por</TH><TH>N° Serie</TH><TH>Estado</TH>
          <TH>Vencimiento</TH><TH>Observaciones</TH><TH>Fecha carga</TH><TH>Acciones</TH>
        </tr>
      </thead>
      <tbody className="divide-y divide-blue-50">
        {items.map((i: any) => (
...
function PdaTable({ items, onDelete, onEdit }: any) {
  return (
    <table className="w-full">
      <thead className="bg-blue-50/70 border-b border-blue-100">
        <tr>
          <TH>Cargado por</TH><TH>Estado</TH><TH>Marca/Modelo</TH>
          <TH>N° Sistemas</TH><TH>N° Línea</TH><TH>Empresa</TH>
          <TH>Asignada a</TH><TH>Obs.</TH><TH>Fecha</TH><TH>Acciones</TH>
        </tr>
      </thead>
      <tbody className="divide-y divide-blue-50">
        {items.map((i: any) => (
...
function InformaticaTable({ items, onDelete, onEdit }: any) {
  return (
    <table className="w-full">
      <thead className="bg-blue-50/70 border-b border-blue-100">
        <tr>
          <TH>Cargado por</TH><TH>Dispositivo</TH><TH>Estado</TH>
          <TH>Marca/Modelo</TH><TH>N° Sistemas</TH><TH>Observaciones</TH>
